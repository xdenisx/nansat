...
class Mosaic(Nansat):
    ...
    def _average_one_file(self, fQueue, matQueue):
        ...
        while True:
            ...
            cntMatTmp[mask == 64] = 1
            avgMatTmp = np.zeros((len(self.bandNames),
                                  dstShape[0], dstShape[1]), 'float16')
            stdMatTmp = np.zeros((len(self.bandNames),
                                  dstShape[0], dstShape[1]), 'float16')
    ...
    def _set_defaults(self, idict):
        ...
        Look throught default parameters (in self) and
        given parameters (dict) and paste value from input
        if the key matches
